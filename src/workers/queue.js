...
class QueueWorker {
  ...
  enqueue(name, task) {
    ...
    console.log(`[Queue] [ENQUEUED] Tarea encolada: ${name}. En espera: ${this.queue.length}`);
    this.processNext();
  }
  ...
  async processNext() {
    ...
    console.log(`[Queue] [PROCESSING] Procesando en paralelo: ${name}... (Activos: ${this.activeCount})`);
    
    try {
      await task();
      console.log(`[Queue] [SUCCESS] Finalizado: ${name}`);
    } catch (err) {
      console.error(`[Queue] [ERROR] Error en tarea ${name}:`, err.message);
    } finally {
      this.activeCount--;
