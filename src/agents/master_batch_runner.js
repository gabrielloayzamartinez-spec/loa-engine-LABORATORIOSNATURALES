...
async function fetchWithRetry(url, options, attempt = 1) {
  ...
  if (now < batchRateLimitBlockedUntil) {
    ...
    console.log(`[RATE_LIMIT] [INFO] Pausa preventiva activa. Esperando ${Math.ceil(waitMs / 1000)}s...`);
    await sleep(waitMs);
  }
...
export async function runMasterSweep(options = {}) {
  console.log("\n============================================================");
  console.log("[BATCH_RUNNER] EJECUTOR MAESTRO DE AUDITORÍA Y DISTRIBUCIÓN 1 POR 1");
  ...
  console.log(`[BATCH_RUNNER] Creado archivo de reporte: ${csvFilename}\n`);

  let url = `https://services.leadconnectorhq.com/contacts/?locationId=${locationId}&limit=100`;
  const allContacts = [];

  console.log("[BATCH_RUNNER] Descargando lista completa de contactos desde GoHighLevel...");
  let pageCount = 0;
  ...
  console.log(`\n\n[BATCH_RUNNER] Total de contactos a auditar y distribuir: ${allContacts.length}\n`);
  ...
  for (let i = 0; i < allContacts.length; i += CONCURRENCY) {
    ...
    if (progress % 50 === 0 || progress === allContacts.length) {
      console.log(`[BATCH_RUNNER] [PROGRESO ${percent}%] ${progress}/${allContacts.length} auditados (${elapsedSec}s) | Intake: ${countIntake} | X1: ${countX1} | X2: ${countX2} | X3: ${countX3} | X4+: ${countX4Plus} | Descuentos: ${totalClicksDiscountable} leads | Calificados: ${calificadosCount}...`);
    }
    await sleep(120);
  }

  console.log(`\n============================================================`);
  console.log(`[BATCH_RUNNER] AUDITORÍA Y DISTRIBUCIÓN MAESTRA 100% COMPLETADA:`);
  console.log(`============================================================`);
  console.log(`[BATCH_RUNNER] Total Contactos Procesados: ${totalProcessed}`);
  console.log(`[BATCH_RUNNER] Base General / Orgánicos (Intake): ${countIntake}`);
  console.log(`[BATCH_RUNNER] Leads Nuevos Únicos (X1): ${countX1}`);
  console.log(`[BATCH_RUNNER] Reingresos X2 (Desc. 1): ${countX2}`);
  console.log(`[BATCH_RUNNER] Reingresos X3 (Desc. 2): ${countX3}`);
  console.log(`[BATCH_RUNNER] Saturación X4+ (Desc. 3+): ${countX4Plus}`);
  console.log(`[BATCH_RUNNER] Pipeline Comercial: Calificados con Tel: ${calificadosCount} | Precalificados Chat: ${precalificadosCount}`);
  console.log(`[BATCH_RUNNER] TOTAL LEADS A DESCONTAR A LA AGENCIA: ${totalClicksDiscountable} leads`);
  console.log(`[BATCH_RUNNER] Reporte CSV consolidado en: ${csvFilename}`);
  console.log(`============================================================\n`);
}
...
export async function runPriorityUnreadSweep() {
  console.log("\n============================================================");
  console.log("[AGENTE_2] INICIANDO BARRIDO PRIORITARIO CON RATE-LIMIT SHIELD");
  ...
  for (const key of order) {
    ...
    console.log(`\n[AGENTE_2] Auditando bandeja de: ${advisor.name}...`);
    ...
    while (true) {
      ...
      if (convs.length === 0) {
        console.log(`[AGENTE_2] Bandeja de ${advisor.name} al día.`);
        break;
      }

      console.log(`[AGENTE_2] Página ${page}: Procesando ${convs.length} chats no leídos de ${advisor.name}...`);
      ...
      if (checkRes.status === 200) {
        ...
        console.log(`[AGENTE_2] Restantes no leídos en ${advisor.name}: ${remaining}`);
  ...
  console.log("\n============================================================");
  console.log(`[AGENTE_2] BARRIDO PRIORITARIO DE BANDEJAS COMPLETADO`);
