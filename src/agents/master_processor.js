...
export async function processMasterContact(contactInput, options = {}) {
  ...
  try {
    ...
    if (!isSilent) {
      console.log(`[MASTER_PROCESSOR] [PROCESSED] ${fullName} | Sede: ${pageLabel} | Asesor: ${targetAdvisorName || 'No asignado'} | Clics: ${totalAdClicks} (${classificationLabel}) | Tel: ${hasPhone ? 'SÍ' : 'NO'}`);
    }
    ...
    if (global.pushLiveLog) {
      ...
      global.pushLiveLog(`[MASTER_PROCESSOR] Procesado: ${fullName} -> ${stageName}`);
    }
