...
export async function runSupervisorAuditor() {
  try {
    ...
    for (const contact of contacts) {
      ...
      if (!contact.phone) {
        warnings.push("[AUDITOR] [ERROR] [Agente 1] Falla: Lead ingresado sin número de teléfono. Comercialmente inútil o proveniente de vTiger incompleto.");
      }
      
      const emailStr = String(contact.email || '');
      if (!emailStr || emailStr.includes('vtigermigrated.com')) {
         if (!contact.phone) {
           warnings.push("[AUDITOR] [ERROR] [Agente 1] Falla: Lead fantasma. No tiene Teléfono ni Correo electrónico real.");
         }
      }

      // 2. Auditoría de Asignación (El Agente 3 hizo su trabajo?)
      if (!contact.assignedTo) {
         warnings.push("[AUDITOR] [ERROR] [Agente 3] Falla: El lead no tiene un asesor asignado. El enrutador falló o no detectó la sede.");
      }

      // 3. Auditoría de Etiquetas
      const tags = contact.tags || [];
      if (tags.length === 0) {
         warnings.push("[AUDITOR] [ERROR] [Agente 1/3] Falla: El lead no tiene ninguna etiqueta. No pasó por el embudo de ruteo.");
      } else {
         const hasPageTag = tags.some(t => t.includes('redes') || t.includes('palacios') || t.includes('bio'));
         if (!hasPageTag) {
            warnings.push("[AUDITOR] [WARN] [Agente 3] Alerta: No se detectó una etiqueta de sede o página clara (Ej: redes-benavides).");
         }
      }
      ...
      if (isMetaSource && !hasAdId) {
         warnings.push("[AUDITOR] [WARN] [Agente 4] Alerta: Lead marcado como Meta Ads/Facebook pero NO se inyectó el Ad ID. El webhook falló o fue contacto orgánico sin clasificar.");
      }
      ...
      if (warnings.length > 0) {
        ...
        if (notesRes.status === 200) {
           ...
           const hasSupervisorNote = existingNotes.some(n => n.body && n.body.includes('SUPERVISOR / AGENTE 5'));
           
           if (!hasSupervisorNote) {
              const noteBody = `SUPERVISOR / AGENTE 5 (Reporte de Salud de Lead)\n===================================\nSe han detectado las siguientes discrepancias en el procesamiento de este Lead:\n\n` + warnings.join('\n\n') + `\n\nPor favor, corregir manualmente si es necesario.`;
              ...
              if (global.pushLiveLog) global.pushLiveLog(`[AUDITOR] [INFO] Anomalías reportadas para ${contact.name || 'Lead'}`);
           }
        }
      }
