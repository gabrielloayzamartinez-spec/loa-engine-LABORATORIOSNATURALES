...
// PODA AUTOMATICA: Evitar memory leak con 300K+ contactos
...
setInterval(runUnassignedConversationsGuardian, 60000);
...
const server = app.listen(PORT, '0.0.0.0', async () => {
  ...
  console.log(`[STATUS] Sedes Operativas: ${getActiveSedes().filter(s => !s.isPaused).map(s => s.sedeId).join(', ') || 'NINGUNA'} | Pausadas (Rate Limit 429): ${getActiveSedes().filter(s => s.isPaused).map(s => s.sedeId).join(', ') || 'NINGUNA'}`);
