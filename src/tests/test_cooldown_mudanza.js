/**
 * [TEST] TEST DE VALIDACIÓN: TIEMPO DE GRACIA (4 DÍAS SIN VENTA / 30 DÍAS CON VENTA), MUDANZA Y SANITIZACIÓN
 * 
 * Reglas de Negocio:
...
export function runCooldownMudanzaSanityCheck() {
  console.log('[TEST] [TEST] Verificando Reglas de Cooldown (4 Días Lead / 30 Días Venta), Mudanza y Sanitización...');
  ...
  if (!test1.cooldownBlocked || test1.finalAdvisorId !== 'advisor_benavides') {
    throw new Error(`[FAIL] Test 1 Falló: Cliente CON VENTA dentro del mes debió ser protegido en su sede vendedora.`);
  }
  console.log('  [PASS] Test 1 Superado: Cliente con venta protegido dentro de su periodo de gracia de 1 mes (15 días).');
  ...
  if (test2.cooldownBlocked || test2.finalAdvisorId !== 'advisor_palacios') {
    throw new Error(`[FAIL] Test 2 Falló: Cliente CON VENTA tras más de 1 mes sin recompra debió mudarse a la nueva sede.`);
  }
  console.log('  [PASS] Test 2 Superado: Cliente con venta se muda legítimamente tras expirar 1 mes sin recompra (40 días).');
  ...
  if (!test3.cooldownBlocked || test3.finalAdvisorId !== 'advisor_palacios') {
    throw new Error(`[FAIL] Test 3 Falló: Lead sin venta en período de gracia (48h) no debió mudarse.`);
  }
  console.log('  [PASS] Test 3 Superado: Lead sin venta protegido dentro de sus 4 días de gracia (48h).');
  ...
  if (test4.cooldownBlocked || test4.finalAdvisorId !== 'advisor_benavides') {
    throw new Error(`[FAIL] Test 4 Falló: Lead sin venta tras 120h debió permitirse la mudanza.`);
  }
  console.log('  [PASS] Test 4 Superado: Mudanza legítima de prospecto sin venta tras expirar 4 días (120h).');
  ...
  if (estadoComercial !== 'SIN VENTA' || fechaAsignacion !== today || fechaCompra !== '' || precioVenta !== '') {
    throw new Error(`[FAIL] Test 5 Falló: Prospecto sin venta no sanitizó campos de compra o no asignó Fecha Ultima Asignacion.`);
  }
  console.log('  [PASS] Test 5 Superado: Prospecto SIN VENTA purga fechas falsas y registra Fecha Última Asignación.');
  ...
  if (estadoComercialSold !== 'CONVERTIDO' || fechaCompraSold !== '2024-03-15') {
    throw new Error(`[FAIL] Test 6 Falló: Cliente CON VENTA no preservó fecha real de compra.`);
  }
  console.log('  [PASS] Test 6 Superado: Cliente CON VENTA preserva fecha de compra real legítima.');

  console.log('[SUCCESS] [TEST PASSED] Todas las reglas de Cooldown (4 Días Lead / 30 Días Venta), Mudanza y Blindaje aprobadas al 100%.');
  return true;
}
