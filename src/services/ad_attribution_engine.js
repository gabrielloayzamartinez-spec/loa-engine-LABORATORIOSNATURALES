...
export async function auditAdAttribution(contactId, options = {}) {
  ...
  if (!isSilent) console.log(`\n[AD_ATTRIBUTION] Auditando pauta para contacto ID: ${contactId}...`);
  ...
  try {
    ...
    for (let i = 0; i < rawAdInteractions.length; i++) {
      ...
      if (distinctAdTouches.length === 0) {
        ...
        // [FILTRO DE RAFAGA TECNICA]: Si llegan con menos de 15 minutos en la misma sede/anuncio, es la misma interacción
...
export async function runHistoricalAdAttributionSweep() {
  console.log("\n=================================================");
  console.log("[AD_ATTRIBUTION] PEINADO HISTÓRICO DE AUDITORÍA MULTI-TOUCH (BATCH)");
  ...
  console.log("[AD_ATTRIBUTION] Extrayendo lista completa de contactos desde GHL...");
  while (url) {
    try {
  ...
  console.log(`\n\n[AD_ATTRIBUTION] Total de contactos a auditar: ${allContacts.length}\n`);
