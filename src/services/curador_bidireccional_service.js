...
export async function runForwardCure(sedeName = 'BENAVIDES', options = {}) {
  ...
  try {
    ...
    const cRes = await fetch(contactsUrl, { headers });
    if (cRes.status === 200) {
    ...
    try {
      ...
      const convRes = await fetch(convUrl, { headers: { ...headers, 'Version': '2021-04-15' } });
...
export async function runBackwardCure(sedeName = 'BENAVIDES', options = {}) {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      throw new Error(`GHL API retornó status ${res.status}`);
    }
  } catch (err) {
