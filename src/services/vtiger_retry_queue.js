...
export function enqueueVtigerRetry(contactId) {
  ...
  if (!queue.includes(contactId)) {
    ...
    saveQueue(queue);
    console.log(`[vTiger Retry Queue] ⏳ Contacto ${contactId} encolado para reintento.`);
  }
}
...
export async function processVtigerRetryQueue() {
  ...
  if (health.status !== 'OK') {
    console.log(`[vTiger Retry Queue] ❌ vTiger inaccesible. Ignorando ${queue.length} contactos en cola por ahora.`);
    return;
  }

  isProcessingQueue = true;
  console.log(`[vTiger Retry Queue] 🚀 Procesando ${queue.length} contactos encolados...`);
  ...
  for (const contactId of queue) {
    try {
      ...
      if (result.success) {
        ...
        console.log(`[vTiger Retry Queue] ✅ Contacto ${contactId} procesado con éxito en el reintento.`);
      }
    } catch (err) {
      console.error(`[vTiger Retry Queue] ⚠️ Error reintentando ${contactId}:`, err.message);
    }
  ...
  if (successful.length > 0) {
    console.log(`[vTiger Retry Queue] 🎉 Se resolvieron ${successful.length} contactos. Quedan ${remaining.length} en cola.`);
  }
