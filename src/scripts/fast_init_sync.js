...
async function fetchWithRetry(url, options, attempt = 1) {
  ...
  try {
    ...
    if (res.status === 429) {
      console.warn(`[Fast Init Shield] [WARN] GHL retornó 429. Pausando workers por 60s...`);
...
async function runFastInit() {
  console.log("\n============================================================");
  console.log("[START] INICIANDO ACELERADOR MASIVO A PRESIÓN (FAST INIT)");
  ...
  console.log("[IMPORT] Fase 1: Descargando lista completa de contactos...");
  let pageCount = 0;
  ...
  console.log(`\n\n[METRICS] Total de contactos a procesar: ${allContacts.length}\n`);
  
  if (allContacts.length === 0) return;

  console.log(" Fase 2: Inyectando presión de inicialización...");
  ...
  console.log(`\n\n[OK] ACELERADOR MASIVO COMPLETADO. Tiempo total: ${((Date.now() - startTime) / 1000 / 60).toFixed(1)} minutos.`);
}
