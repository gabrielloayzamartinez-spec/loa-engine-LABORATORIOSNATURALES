...
export async function getOrCreateCustomFields() {
  console.log("\n[SCAN] Verificando Campos Personalizados en GHL...");
  ...
  for (const def of CUSTOM_FIELDS_DEF) {
    ...
    if (found) {
      console.log(`[OK] Campo detectado: "${found.name}" (ID: ${found.id})`);
      createdFields.push(found);
      continue;
    }

    console.log(`[START] Creando Campo Personalizado: "${def.name}" (${def.dataType})...`);
    try {
      const payload = {
        name: def.name,
      ...
      console.log(`[SUCCESS] ¡CAMPO "${created.name}" CREADO CON ÉXITO! (ID: ${created.id || 'N/A'})`);
      createdFields.push(created);
    } catch (err) {
      console.error(`[ERROR] Error creando Campo "${def.name}":`, err.message);
    }
  }
