...
async function forceResetAllToIntake() {
  console.log("\n============================================================");
  console.log("[ALERT] RESET TOTAL: MOVIENDO TODAS LAS OPORTUNIDADES A INTAKE");
  console.log(` Pipeline ID: ${PIPELINE_ID}`);
  console.log(`[IMPORT] Etapa Destino: [IMPORT] Intake / Base Completa (${INTAKE_STAGE_ID})`);
  console.log("============================================================\n");

  console.log("[SCAN] Escaneando oportunidades que NO estén en Intake...");
  const oppsToMove = [];
  let page = 1;
  ...
  console.log(`\n\n[TARGET] Oportunidades encontradas fuera de Intake: ${oppsToMove.length}`);

  if (oppsToMove.length === 0) {
    console.log("[OK] ¡El 100% de las oportunidades ya están dentro de la columna de Intake!");
    return;
  }

  console.log(`\n[START] Moviendo las ${oppsToMove.length} oportunidades a la columna de Intake en paralelo...`);
  ...
  console.log(`\n\n[SUCCESS] ¡COMPLETADO! Las ${movedCount} oportunidades fueron devueltas a la primera columna "[IMPORT] Intake / Base Completa".\n`);
}
