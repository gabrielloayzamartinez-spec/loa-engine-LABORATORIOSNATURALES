...
async function main() {
  ...
  for (const s of sedes) {
    ...
    if (targetMode === 'forward' || targetMode === 'both') {
      ...
      console.log(`[OK] Resultado Forward en ${s}:`, resForward);
    }
    ...
    if (targetMode === 'backward' || targetMode === 'both') {
      ...
      for (let cycle = 1; cycle <= maxCycles; cycle++) {
        ...
        console.log(`[OK] Resultado Backward (Ciclo ${cycle}):`, resBackward);
        if (resBackward?.isCompleted) {
          console.log(`[SUCCESS] ¡Barrido histórico completado al 100% para ${s}!`);
          break;
        }
      }
    }
  ...
  console.log('\n========================================================================');
  console.log('[METRICS] MÉTRICAS CONSOLIDADAS DEL CURADOR BI-DIRECCIONAL:');
