...
async function healBatch() {
  ...
  try {
    ...
    if (contacts.length === 0) {
      console.log(`[HISTORIAL HEALER] [OK] ¡Base de datos completamente escaneada y curada!`);
      return; // Fin
    }
    ...
    for (let c of contacts) {
      ...
      if (!existingTags.includes('historial-vtiger-sincronizado')) {
         try {
            ...
            console.error(`[HISTORIAL HEALER] [WARN] Error sincronizando vTiger para ${c.id}: ${e.message}`);
         }
      }
      // -------------------------------------------------------------
...
console.log(`[WAIT] Ritmo: ${BATCH_SIZE} contactos cada 5 minutos.`);
console.log(`==========================================================\n`);
