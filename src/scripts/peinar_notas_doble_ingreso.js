/**
 * [CLEAN] PEINADO Y ACTUALIZACIÓN HISTÓRICA DE NOTAS Y DOBLE INGRESO PUBLICITARIO
 * 
 ...
 *    -  CLIENTE COMPRADOR (con compras en vTiger/GHL)
 *    - [PHONE] PROSPECTO CALIFICADO (con teléfono para despacho)
 *    - [CHAT] CURIOSO (solo chat preliminar)
 * 5. Clasificar intereses clínicos ([WARN] MULTICONSULTA si pregunta por 3+ tratamientos).
...
async function fetchWithRetry(url, options, attempt = 1) {
  try {
    ...
    if (res.status === 429) {
      ...
      console.log(`\n[WAIT] [Rate Limit 429] Esperando ${wait / 1000}s para reanudar...`);
      await sleep(wait);
...
export async function runPeinadoNotasDobleIngreso(maxContacts = 100) {
  console.log('\n=======================================================================');
  console.log('[CLEAN] INICIANDO PEINADO HISTÓRICO: FICHA LIMPIA Y DOBLE INGRESO');
  ...
  console.log(`[DOC] Reporte CSV inicializado en: ${csvReportPath}\n`);
  ...
  console.log(`[IMPORT] Extrayendo contactos actualizados recientemente desde GoHighLevel (límite: ${maxContacts})...`);
  ...
  console.log(`\n\n[TARGET] Total de contactos a procesar: ${contactsToAudit.length}\n`);
  ...
  for (let i = 0; i < contactsToAudit.length; i++) {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        shieldedCount++;
        process.stdout.write(`\r[${contactNum}/${contactsToAudit.length}] [SHIELD] Blindaje activo: Asesor en chat reciente con contacto ID: ${c.id}`);
      }
    } catch (err) {
      console.error(`\n[ERROR] Error en contacto ${c.id}: ${err.message}`);
    }
  }

  console.log('\n\n=======================================================================');
  console.log('[SUCCESS] RESUMEN DEL PEINADO Y ACTUALIZACIÓN DE FICHAS:');
  console.log('=======================================================================');
  console.log(`[USERS] Total Contactos Procesados: ${processedCount}`);
  console.log(`[STAR] Dobles Ingresos Válidos (Multiproducto/Multisede/Tiempo): ${doubleEntriesCount}`);
  console.log(`[GREEN] Leads Nuevos Únicos (X1): ${singleLeadsCount}`);
  console.log(`[YELLOW] Reingresos Mismo Producto: ${reentriesCount}`);
  console.log(`[SHIELD] Contactos Protegidos por Blindaje de Asesor (15 min): ${shieldedCount}`);
  console.log(`[DOC] Archivo de auditoría generado: ${csvReportPath}`);
  console.log('=======================================================================\n');
