...
async function sweepAdvisorUnread(advisorKey, advisor) {
  console.log(`\n=============================================================`);
  console.log(`[CLEAN] BARRIDO MASIVO: ${advisor.name} (ID: ${advisor.id})`);
  console.log(`=============================================================`);
  ...
  while (nextPageUrl && hasMore) {
    ...
    if (convs.length === 0) {
      console.log(`[OK] No hay más conversaciones no leídas para ${advisor.name}.`);
      break;
    }

    console.log(`[BATCH] Procesando lote de ${convs.length} conversaciones no leídas...`);
    ...
    if (checkRes.status === 200) {
      ...
      console.log(`[METRICS] Restantes no leídos en ${advisor.name}: ${remaining}`);
  ...
  console.log(`[SUCCESS] Finalizado barrido de ${advisor.name}. Total auditados/corregidos: ${processed}`);
}

async function main() {
  console.log("[START] INICIANDO BARRIDO GENERAL DE BANDEJAS NO LEÍDAS (TODAS LAS SEDES)");
  ...
  console.log("\n=============================================================");
  console.log("[FINISH] ¡BARRIDO COMPLETO FINALIZADO EN TODAS LAS BANDEJAS!");
  console.log("=============================================================\n");
}
