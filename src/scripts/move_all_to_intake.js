...
async function moveAllRadarOpportunitiesToIntake() {
  console.log("\n============================================================");
  console.log("[START] TRASLADO MASIVO DE OPORTUNIDADES HACIA LA COLUMNA DE INTAKE");
  console.log(` Pipeline ID: ${PIPELINE_ID}`);
  console.log(`[IMPORT] Etapa Destino: [IMPORT] Intake / Base Completa (${INTAKE_STAGE_ID})`);
  console.log("============================================================\n");

  console.log("[IMPORT] Obteniendo todas las oportunidades del Radar...");
  const allOpps = [];
  let page = 1;
  ...
  console.log(`\n\n[METRICS] Total de oportunidades en el Radar: ${allOpps.length}`);
  ...
  if (oppsToMove.length === 0) {
    console.log("[OK] Todas las oportunidades ya están en la columna de Intake.");
    return;
  }
  ...
  console.log(`\n\n[SUCCESS] ¡TRASLADO COMPLETO! ${movedCount} oportunidades movidas a la primera columna "[IMPORT] Intake / Base Completa".\n`);
}
