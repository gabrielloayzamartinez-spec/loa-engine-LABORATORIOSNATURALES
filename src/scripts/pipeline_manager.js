...
export async function getOrCreateMasterPipeline(existingPipelines = null) {
  console.log("\n[SCAN] Verificando existencia del Pipeline Maestro (Ventas)...");
  ...
  if (found) {
    console.log(`[OK] Pipeline Maestro detectado: "${found.name}" (ID: ${found.id})`);
    return formatMasterPipelineResult(found);
  }

  console.log(`[START] Creando "${MASTER_PIPELINE_DEF.name}" con ${MASTER_PIPELINE_DEF.stages.length} etapas...`);
  try {
    const payload = {
    ...
    console.log(`[SUCCESS] ¡PIPELINE MAESTRO CREADO CON ÉXITO! (ID: ${created.id})`);
    return formatMasterPipelineResult(created);
  } catch (err) {
    console.error("[ERROR] Error creando Pipeline Maestro:", err.message);
    throw err;
  }
}

export async function getOrCreateAuditPipeline(existingPipelines = null) {
  console.log("\n[SCAN] Verificando existencia del Pipeline de Auditoría Multi-Touch...");
  const pipelines = existingPipelines || await fetchPipelines();
  ...
  if (found) {
    console.log(`[OK] Pipeline de Auditoría detectado: "${found.name}" (ID: ${found.id})`);
    return formatAuditPipelineResult(found);
  }

  console.log(`[START] Creando "${AUDIT_PIPELINE_DEF.name}" con ${AUDIT_PIPELINE_DEF.stages.length} etapas...`);
  try {
    const payload = {
    ...
    const createData = await createRes.json();
    const created = createData.pipeline || createData;
    console.log(`[SUCCESS] ¡PIPELINE DE AUDITORÍA CREADO CON ÉXITO! (ID: ${created.id})`);
    return formatAuditPipelineResult(created);
  } catch (err) {
    console.error("[ERROR] Error creando Pipeline de Auditoría:", err.message);
    throw err;
  }
}
...
export async function getOrCreateUnifiedPipeline(existingPipelines = null) {
  console.log("\n[SCAN] Verificando existencia del Pipeline Unificado...");
  const pipelines = existingPipelines || await fetchPipelines();
  
  const found = pipelines.find(p => p.name === UNIFIED_PIPELINE_DEF.name);
  if (found) {
    console.log(`[OK] Pipeline Unificado detectado: "${found.name}" (ID: ${found.id})`);
    return formatUnifiedPipelineResult(found);
  }

  console.log(`[START] Creando "${UNIFIED_PIPELINE_DEF.name}" con ${UNIFIED_PIPELINE_DEF.stages.length} etapas...`);
  try {
    const payload = {
    ...
    const createData = await createRes.json();
    const created = createData.pipeline || createData;
    console.log(`[SUCCESS] ¡PIPELINE "${created.name}" CREADO CON ÉXITO! (ID: ${created.id})`);
    return formatUnifiedPipelineResult(created);
  } catch (err) {
    console.error(`[ERROR] Error creando Pipeline Unificado:`, err.message);
    throw err;
  }
}
