...
export const PALACIOS_TEMPLATES = [
  ...
  },
  {
    ...
    body: '¡Hola {{contact.first_name}} , sabemos que cuando algo afecta nuestra vitalidad o rendimiento, también puede influir en cómo nos sentimos día a día, déjanos tu NÚMERO para poder ayudarte y enviarte tu MUESTRA GRATIS [HOT]'
  },
  {
  ...
  },
  {
    ...
    body: '¡Hola {{contact.first_name}}  Sabemos que cuando algo afecta nuestra vitalidad o rendimiento, también puede influir en cómo nos sentimos día a día. Déjanos tu número de teléfono para poder ayudarte y coordinar tu envío. [HOT]'
  },
  {
  ...
  },
  {
    ...
    body: '{{contact.first_name}} [WARN] ¡Últimas unidades disponibles!\n Es tu oportunidad para apoyar tu vitalidad y energía.\n[BATCH] Déjanos tu número de teléfono y el lugar de envío para asegurar tu entrega'
  },
  {
  ...
  },
  {
    ...
    body: '[STAR]¡Tu envío está casi listo![STAR]\n No dejes pasar esta oportunidad de apoyar el cuidado de tu salud. Déjanos tu número de teléfono y el lugar a donde enviaremos tu pedido para programar el envío. [BATCH]'
  },
  ...
  },
  {
    ...
    body: '¿Nos podemos comunicar contigo para ayudarte? [PHONE]'
  },
  {
  ...
  },
  {
    ...
    body: 'Para terminar el registro de tu MUESTRA GRATIS, indícame tu número de teléfono y el lugar de envio para así poder coordinar la entrega  '
  }
];

export async function injectPalaciosTemplates() {
  console.log("\n=================================================");
  console.log("[START] INYECTOR DE PLANTILLAS PARA PALACIOS EN GHL");
  ...
  for (let i = 0; i < PALACIOS_TEMPLATES.length; i++) {
    ...
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        console.log(`  [OK] Creada con éxito (ID: ${data.template?.id || data.id || 'OK'})`);
        successCount++;
      } else {
        const errText = await res.text();
        console.error(`  [ERROR] Error (${res.status}): ${errText}`);
        errorCount++;
        if (res.status === 401) {
          console.log("\n[WARN] DETENIDO: El Token de GHL requiere el permiso 'templates.write'.");
          console.log("Ve a GHL > Settings > Developers > Private Integrations > Editar Token > Marcar 'Templates (Write)'.");
          break;
        }
      }
    } catch (e) {
      console.error(`  [ERROR] Excepción: ${e.message}`);
      errorCount++;
    }
  ...
  console.log("\n=================================================");
  console.log(`[METRICS] RESULTADO FINAL:`);
  console.log(`[OK] Creadas con éxito: ${successCount}`);
  console.log(`[ERROR] Fallidas / Pendientes: ${errorCount}`);
  console.log("=================================================\n");
}
