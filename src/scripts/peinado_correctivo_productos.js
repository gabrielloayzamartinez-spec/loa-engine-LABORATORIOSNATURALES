...
export async function runPeinadoCorrectivo(limitContacts = 100) {
  console.log('===============================================================');
  console.log('[CLEAN] INICIANDO PEINADO CORRECTIVO DE PRODUCTOS Y FUENTES (GHL)');
  console.log('===============================================================');
  ...
  while (nextPageUrl && totalScanned < limitContacts) {
    ...
    for (const c of contacts) {
      ...
      if (realTreatment && realTreatment !== 'Artritis' && (hasArtritisTag || sourceIsArtritis)) {
        console.log(`\n[ALERT] DETECTADO FALSO POSITIVO: ${c.firstName || ''} ${c.lastName || ''} (${c.id})`);
        ...
        if (updateRes.status === 200) {
          totalFixed++;
          console.log(`   [OK] CORREGIDO CON ÉXITO -> Fuente: ${newSource} | Tag: ${correctTag}`);
        } else {
          console.error(`   [ERROR] Falló actualización de ${c.id}: HTTP ${updateRes.status}`);
        }
  ...
  console.log('\n===============================================================');
  console.log(`[FINISH] PEINADO FINALIZADO: ${totalScanned} contactos auditados, ${totalFixed} corregidos.`);
  console.log('===============================================================');
}
