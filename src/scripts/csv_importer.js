...
async function processFile(filePath, startRow) {
  console.log(`\n[DOC] Procesando archivo: ${path.basename(filePath)} desde fila ${startRow}...`);
...
async function main() {
  ...
  if (globalFileIndex >= files.length) {
    console.log("[OK] Importación completada. Todos los archivos procesados.");
    return;
  }

  console.log(`[START] Iniciando Agente 2 (Importador Histórico)...`);
  ...
  console.log("[SUCCESS] INYECCION MASIVA COMPLETADA");
}
